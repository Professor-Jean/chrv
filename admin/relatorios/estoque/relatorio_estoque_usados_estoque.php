<div>
  <span class="imprimir">
    <h1 class="titulo_corpo" align="center">Relatório de Estoque de Produtos Usados</h1>
  </span>
  <a href="?folder=../../&file=inicial_admin.php"><button class="tipos">Produtos Novos</button></a>
  ...
  <span class="imprimir">
    <table class="table_relatorio">
      <?php
        $sql_sel_estoque = "SELECT produtos.id, produtos.nome AS produto, produtos.imagem, marcas.nome AS marca, SUM(entradas.quantidade) AS soma_quantidade, entradas.valor_compra, entradas.despesas, entradas.margem, entradas.frete, entradas.st, entradas.ipi FROM entradas INNER JOIN produtos ON entradas.produtos_id=produtos.id INNER JOIN marcas ON produtos.marcas_id=marcas.id WHERE entradas.tipo='PU' AND entradas.esgotada='0' GROUP BY produtos.id ASC";
      ...
      <tr>
        ...
        <td class="td_relatorio"><?php echo $sql_sel_estoque_dados['soma_quantidade']; ?></td>
        <td class="td_relatorio"><?php echo "R$".number_format($valor_venda,2,',','.'); ?></td>
        <td class="td_relatorio"><a style="color: #000;" href="?folder=relatorios/estoque/informacoes/usados/&file=informacoes_produto_informacoes&ext=php&id=<?php echo $sql_sel_estoque_dados['id']; ?>"><i class="fa fa-info-circle" aria-hidden="true"></i></a></td>
      <?php
