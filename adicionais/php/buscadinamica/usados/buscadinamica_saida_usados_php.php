...
*/
	...
	if($tipo=='1'){
		...
		$sql_sel_entradas="SELECT entradas.id, entradas.produtos_id, produtos.nome FROM entradas INNER JOIN produtos ON entradas.produtos_id=produtos.id WHERE categorias_id='".$cat."' AND marcas_id='".$mar."' AND entradas.tipo='PU' AND entradas.esgotada='0' GROUP BY entradas.produtos_id ASC";
		...
		while($sql_sel_entradas_dados=$sql_sel_entradas_preparado->fetch()){
			...
			$sql_sel_entradas="SELECT id, produtos_id, tipo, valor_venda, data FROM entradas WHERE produtos_id='".$pro."' AND tipo='PU' AND esgotada='0'";
			$sql_sel_entradas_preparado = $conexaobd->prepare($sql_sel_entradas);
			$sql_sel_entradas_preparado->execute();
			echo "<option value=''>Escolha...</option>";
			while($sql_sel_entradas_dados=$sql_sel_entradas_preparado->fetch()){
